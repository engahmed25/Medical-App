...
export default function BookingSlots() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white">
      <h2 className="text-2xl font-semibold text-(--head-desc-color) mb-6">
        Book an appointment
      </h2>

      {/* Date Selection */}
      <div className="mb-8">
        <h3 className="text-m font-semibold text-(--head-desc-color) mb-3">Select Date</h3>
          ...
          {dates.map((item, index) => (
            <Button
              ...
              className={`flex flex-col items-center justify-center min-w-[55px] md:min-w-[70px] h-20 rounded-xl border-2 transition-all ${
                selectedDate === index
                  ? "!border-(--main-color) !bg-(--main-verylite-color) !text-(--main-color)"
                  : "!bg-(--sec-color)] !border-gray-200 hover:!border-gray-400 !text-[var(--head-desc-color)]"
              }`}
            >
