...
export default function BookingSlots() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-white">
      ...
      {/* Date Selection */}
      <div className="mb-8">
        <h3 className="text-m font-medium text-[var(--head-desc-color)] mb-3">Select Date</h3>
        <div className="flex gap-3 flex-wrap pb-2">
          {dates.map((item, index) => (
            <Button
              key={index}
              onClick={() => setSelectedDate(index)}
              className={`flex flex-col items-center justify-center min-w-[70px] md:min-w-[70px] h-20 rounded-xl border-2 transition-all ${
                selectedDate === index
      ...
      {/* Time Selection */}
      <div className="mb-8">
        <h3 className="text-m font-medium text-gray-700 mb-3">Select Time</h3>
